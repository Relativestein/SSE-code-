# -*- coding: utf-8 -*-
"""SSE algorithm for heisenberg model.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1c5Ub6DxUBxl7AT2NjFJ_fy5yyT22ztPc
"""
...
def measuredata(energy,s_heat):
    energy = energy + nh
    s_heat = s_heat + nh**2
    
    return(energy,s_heat)

def writeresults(energy,s_heat,av_energy,av_s_heat,msteps):

    energy = energy / msteps 
    s_heat = s_heat/ msteps
    s_heat = s_heat - energy*(energy + 1.00) 
    s_heat = s_heat / nos
    energy = energy / (beta * nos) - 0.5
    av_energy = av_energy + energy
    av_s_heat = av_s_heat + s_heat 
    return av_energy, av_s_heat

av_energy = 0
av_s_heat = 0
  
for i in range(0,20,1):
    for j in range(0,msteps,1):
        m,nh,opstring = diagonalupdate(m,nh,opstring)
        vertexlist = vertexlinker(m,opstring,vertexlist)
        opstring,vertexlist,spin = loopupdate(m,vertexlist,opstring,spin)
        energy,s_heat = measuredata(energy,s_heat)
    av_energy, av_s_heat = writeresults(energy,s_heat,av_energy, av_s_heat, msteps)
    energy = 0
    s_heat = 0
av_energy = av_energy / 10
av_s_heat = av_s_heat/10
file1 = open("results.txt","w")
print("E/N:",av_energy,file = file1)
print("C/N:",av_s_heat,file = file1)
file1.close()
